...
ABUSE_STRINGS = [
	   ...
	   "`Ja be Gaandu`",
	   "`chut chaman k chivde`",
	   "`Ja be Chodulund`",
	   "`Teri food mein shivam k jhaat sprinkle honge`",
	   "`Muh Me Lega Bhosdike ?`"
]
GEY_STRINGS = [
     "`you gey bsdk`",
     "`gey toh ek hi hai shivam`",
     "`gey in the house`",
     "`you chakka`",
     "`gey ho kya?`",
     "`you gey go away`",
    ...
    if not hacked.text[0].isalpha() and hacked.text[0] not in ("/", "#", "@", "!"):
        if await hacked.get_reply_message():
            await hacked.edit(
                ...
                "`give your ass to` @shivamkchoudhary `To Remove This Hack...`\n"
            )
...
async def spongemocktext(mock):
    ...
    if not mock.text[0].isalpha() and mock.text[0] not in ("/", "#", "@", "!"):
        ...
        if message:
            ...
            await mock.edit("`Do YoU NeEd a CoCk!`")
            return
...
async def claptext(memereview):
    ...
    if not memereview.text[0].isalpha() and memereview.text[0] not in ("/", "#", "@", "!"):
        ...
        if message:
            ...
            await memereview.edit("`Hasu kya?`")
            return
        reply_text = "👏 "
...
@register(outgoing=True, pattern="^.bhangi$")
async def bluetext(bte):
    ...
    if not bte.text[0].isalpha() and bte.text[0] not in ("/", "#", "@", "!"):
        if await bte.get_reply_message():
            await bte.edit(
                "`BLUETEXT MUST CLICK.`\n"
                "`Are you a bhangi which is attracted to girls?`"
            )
